...
public class InputMethods extends SelectElementByType implements BaseTest
{
	...
	public void CheckText(String accessType,String text,String accessName, boolean testCase) throws TestCaseFailed
	{
		wait.until(ExpectedConditions.presenceOfElementLocated(getelementbytype(accessType, accessName)));
		String value = driver.findElement(getelementbytype(accessType, accessName)).getAttribute("value");
		
		if(testCase)
		{
			if(!value.equals(text))
				throw new TestCaseFailed("Input Actual as : "+value);
		}
		else
		{
			if(value.equals(text))
